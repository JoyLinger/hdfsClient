...
# If the YARN_CONF_DIR looks like /etc/yarn1/conf:/etc/hdfs1/conf:/etc/hbase/conf.
# The following command will fail. So we will cut it with : and get the first one
YARN_CONF_DIR=`echo ${YARN_CONF_DIR} | awk -F ":" '{print $1}'`
